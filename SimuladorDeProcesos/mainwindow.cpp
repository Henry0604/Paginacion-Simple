...
int MainWindow::availableFrames()
{
    ...
    for (i = availableFrames = 0; i < FRAMES - 4; i++){
        if (memory[i].freePages == PAGES){
            availableFrames++;
        }
    }
...
void MainWindow::updateMemoryTable()
{
    ...
    for (i = 40; i < FRAMES; i++){
        ...
        ui->memoryTB->setItem(i,1,new QTableWidgetItem(QString("0/5")));
